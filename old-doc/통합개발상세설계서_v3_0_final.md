# 통합 경정청구 환급액 산출 시스템 — 개발상세설계서 v3.0

> **문서 버전**: v3.0 (v2.0 Final + v2.1 보완 → 아키텍처 전면 개편)  
> **작성일**: 2026-02-16  
> **기준 프롬프트**:  
...
> | 변경 | v2.0/v2.1 | v3.0 |
> |------|----------|------|
> | **① 요청번호 관리** | entity_id(법인ID/납세자ID) 기반 | `req_id` (신청인+날짜별 유니크) 전 테이블 관통 |
> | **② 입력자료 구조** | 항목별 개별 입력 테이블 24+8개 | **원시 JSON 보관 테이블** + **계산용 요약 테이블** 2계층 |
> | **③ 출력결과 구조** | 개별 출력 테이블 10+3+4개 | **결과 저장 테이블** + **JSON 전달 인터페이스** 2계층 |
>
> **변경이력 누적**: 법인세 v1.0→v4.0r1 원문 + 종합소득세 v1.3 신규 + v1.1 보완 12건 + v2.1 출력양식 보강 + v3.0 아키텍처 개편
...
30. [M1 입력 관리 — v3.0 개편 (JSON 수신·파싱·요약 생성)](#30-m1-입력관리)
31. [M2~M5 점검·산출 — v2.0 승계 요약](#31-점검산출-승계)
32. [M6 보고·전달 — v3.0 개편 (결과 저장·JSON 직렬화·API 응답)](#32-m6-보고전달)
33. [단계별 업무처리 절차서 (v3.0)](#33-업무처리-절차서)

### PART X. 계산 공식·검증 규칙·용어 (v2.0 승계)
34. [계산 공식 정의서 — v2.0 전량 승계](#34-계산공식)
35. [검증 규칙 — v2.0 + v2.1 + v3.0 추가](#35-검증규칙)
...
| **이력 추적** | 동일 신청인의 재요청 시 req_id가 달라지므로 버전별 비교 가능 |
...
| **기준정보** | REF_ | 22 | — | 법령·세율·공제율 등 공통 기준 (v2.0 승계) |
...
| 9 | request_status | 요청상태 | VARCHAR(15) | Y | received / parsing / processing / completed / error |
...
| 14 | error_message | 오류메시지 | TEXT | N | error 상태 시 상세 |
...
> employee_detail + employee_monthly 원시 JSON에서 집계한 **환급액 계산에 필요한 핵심 수치**.
...
| 2 | item_seq | 항목순번 | INT | PK | 1~ | — |
| 3 | item_category | 항목구분 | VARCHAR(20) | Y | INVEST/RD/STARTUP/SME_SPECIAL/EMPLOYMENT/SOCIAL_INS/FOREIGN_TAX/INC_SINCERITY/INC_LANDLORD/... | — |
| 4 | provision | 적용조항 | VARCHAR(20) | Y | 예: §24, §10, §6, §7 | — |
| 5 | base_amount | 기초금액 | BIGINT | Y | 투자금액 / R&D비용 / 감면대상소득 등 | 각 category JSON 집계 |
| 6 | zone_type | 소재지구분 | VARCHAR(20) | N | 수도권/비수도권 | 자산 설치장소 / 사업장 소재지 |
| 7 | asset_type | 자산유형 | VARCHAR(50) | N | 투자: 신성장/일반/중고 | investment.asset_type |
| 8 | rd_type | R&D유형 | VARCHAR(30) | N | 국가전략/신성장/일반 | rd_expense.rd_type |
| 9 | sub_detail | 세부내역 | TEXT | N | JSON: 항목 고유 상세 | 원시 JSON에서 필요 필드 추출 |
| 10 | existing_applied | 기존적용여부 | BOOLEAN | N | 기존 신고에서 이미 적용했는지 | existing_deduction 대조 |
| 11 | existing_amount | 기존적용금액 | BIGINT | N | | existing_deduction.amount |
| 12 | carryforward_balance | 이월잔액 | BIGINT | N | 이월세액공제 잔액 | credit_carryforward 집계 |

**복합 PK**: `(req_id, item_seq)`
...
> **req_id 없이 독립 관리**. v2.0 PART II §4.1~4.22 전량 승계. 변경 없음.
...
| 20 | calc_detail | 산출상세JSON | TEXT | N | 산출 과정 상세 |
| 21 | legal_basis | 근거법령 | VARCHAR(100) | Y | |
...
| 6 | items_json | 포함항목상세 | TEXT | Y | JSON: [{item_id, provision, amount}] |
...
| 20 | optimal_combo_id | 최적조합ID | INT | FK | → OUT_COMBINATION |
| 21 | carryforward_credits | 이월공제잔액 | BIGINT | N | |
| 22 | carryforward_detail | 이월공제상세 | TEXT | N | JSON |
| 23 | penalty_tax_change | 가산세변동 | BIGINT | N | |
...
| 13 | checksum | 체크섬 | VARCHAR(64) | N | SHA-256 |
| 14 | generated_at | 생성일시 | TIMESTAMP | Y | |
...
|----|--------|------|------|------|------|
| **API-01** | **POST** | `/api/v1/requests` | **요청 접수 + 원시 JSON 수신** | 입력 JSON 데이터셋 전체 | `{req_id, status: "received"}` |
| **API-02** | **POST** | `/api/v1/requests/{req_id}/analyze` | **점검 실행** (M2~M6 파이프라인) | — | `{req_id, status: "processing"}` |
...
처리:
  1. applicant_id + today 기준 기존 seq_no 조회
  2. seq_no = MAX(기존) + 1 (없으면 1)
  3. req_id = {applicant_type}-{applicant_id(하이픈제거)}-{YYYYMMDD}-{seq_no:03d}
  4. INSERT INTO REQ_REQUEST (req_id, applicant_type, applicant_id, ..., request_status='received')
  5. RETURN req_id
```
...
처리:
  ...
  UPDATE REQ_REQUEST SET request_status = 'processing'
```
...
처리:
  ...
  ... (v2.0 STEP4 동일 로직)
  INSERT INTO OUT_REFUND (req_id, ...)
  INSERT INTO OUT_RISK (req_id, ...) -- 사후관리
...
>         → v2.1(출력양식 보강) → **v3.0(아키텍처 전면 개편, 현재)**  
