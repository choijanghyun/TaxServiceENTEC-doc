...
> | No | 변경 영역 | 변경 내용 |
> |----|----------|----------|
> | ① | **테이블 명명규칙 개편** | `{주제영역}_{활용구분}_{테이블명}` — 5개 주제영역(RI/SV/CO/AL/RF) + 활용구분(C/I/S) 식별 |
> | ② | **주제영역 5영역 재편** | 요청및입력상세(RI), 요약및검증(SV), 산출결과(CO), 감사로그(AL), 기준정보(RF) |
> | ③ | **신청인·납세자 분리 관리** | RI_S_APPLICANT(신청인/세무대리인) + RI_S_TAXPAYER(납세자=개인/법인사업자) 신설, RI_S_REQUEST에 FK 추가 |
...
38~41. [입력 원시 스키마 컬럼 상세 + 부록 A/B/C](#38-입력-원시-스키마)
...
> 아래는 v2.0에서 전량 승계된 RF_* 테이블의 컬럼 정의입니다. v3.0에서 변경 없음.

### 4.1 RF_S_TAX_RATE — 법인세율 기준
...
### 4.2 RF_S_MIN_TAX_RATE — 최저한세율 기준
...
### 4.3 RF_S_EMPLOYMENT_CREDIT — 통합고용세액공제 금액 기준
...
### 4.4 RF_C_INVESTMENT_CREDIT_RATE — 통합투자세액공제율 기준
...
### 4.5 RF_S_MUTUAL_EXCLUSION — 상호배제 매트릭스
...
### 4.6 RF_S_CAPITAL_ZONE — 수도권 지역 구분
...
### 4.7 RF_S_NONGTEUKSE — 농어촌특별세 적용 기준
...
### 4.8 RF_S_SME_DEDUCTION_RATE — 중소기업 특별세액감면율
...
### 4.9 RF_C_RD_CREDIT_RATE — R&D 세액공제율
...
### 4.10 RF_S_REFUND_INTEREST_RATE — 환급가산금 이율 *(v2.0 신규, M13)*
...
### 4.11 RF_S_STARTUP_DEDUCTION_RATE — 창업감면 감면율 기준 *(v2.0 신규)*
...
### 4.12 RF_S_DEPOPULATION_AREA — 인구감소지역 목록 *(v2.0 신규)*
...
### 4.13 RF_C_TAX_RATE_HISTORY — 법인세율 이력 (2022년 이전 포함) *(v2.0 신규)*
...
### 4.14 RF_S_LAW_VERSION — 법령 버전 관리 *(v2.0 신규, I02)*
...
### 4.15 RF_C_DIVIDEND_EXCLUSION — 수입배당금 익금불산입률 *(v1.1 → v3.0 통합)*
...
### 4.16 RF_C_RD_MIN_TAX_EXEMPT — R&D 최저한세 배제율 *(v1.1 → v3.0 통합)*
...
### 4.17 RF_S_INDUSTRY_ELIGIBILITY — 업종별 감면 적격성 *(v4.0 신규, M26)*
...
### 4.18 RF_C_ENTERTAINMENT_LIMIT — 접대비 한도 기준 *(v4.0 신규, M25)*
...
### 4.19 RF_S_SYSTEM_PARAM — 시스템 파라미터 *(v4.0 신규, M32)*
...
### 4.20 RF_S_KSIC_CODE — 한국표준산업분류 *(v4.0 신규, M35)*
...
### 4.21 RF_S_EXCHANGE_RATE — 외국환 매매기준율 *(v4.0r1 신규)*
...
### 4.22 RF_C_DEEMED_INTEREST_RATE — 인정이자율 기준 *(v4.0r1 신규)*
...
### 5.3 RF_I_DEDUCTION_LIMIT — 소득공제 한도
...
### 5.4 RF_I_SINCERITY_THRESHOLD — 성실신고확인 수입금액 기준
...
# PART III (계속). 점검·검증 결과

---

## 14A. SV_S_PREP_SUMMARY — 사전 데이터 준비 요약 ( 신설)
...
> v2.1 SV_S_ELIGIBILITY 승계 + req_id 적용. 가이드 섹션 A 대응.
...
> v2.1 SV_S_INSPECTION_LOG 승계 + req_id 적용. 가이드 섹션 E 대응.
...
> v2.0 CO_S_CREDIT_DETAIL + v2.1 보강분 통합.
...
> 아래 의사코드에서 `corp_id` → `req_id`, `RI_*` → `SV_*` 으로 읽어야 합니다.
...
###  8.1b M4-11a: 업무용 승용차 비용 재산출 [CORP] (신설)
...
## 33C. 법인세 업무처리 절차서 — STEP별 상세 

> v3.0 §33은 v3.0 파이프라인 요약. 아래는 v2.0의 STEP 0~5 상세 절차입니다.
> 입력 소스만 `SV_*`로 변경, 처리 로직은 동일.
...
### 6.1A PREP 단계: 요약자료 검증 및 기초 데이터 산정 ( 신설)
...
> v3.0 변경사항: 모든 공식의 입력 소스가 `RI_C_CORP_BASIC` → `SV_S_BASIC`, `RI_S_EMPLOYEE_*` → `SV_S_EMPLOYEE` 등으로 변경.
> 공식 자체의 계산 로직은 동일.
...
> v3.0 변경사항: 모든 규칙의 테이블 참조가 `RI_*` → `SV_*` / `CO_*` / `SV_*`로 변경.
> 규칙 로직 자체는 동일.
...
| V16 | 업무용 승용차 감가상각 한도(RI_C_BUSINESS_VEHICLE) = 연 800만원 이하 | WARNING |
...
| V19 | 벤처확인일 - 창업일 ≤ 3년 (RI_S_STARTUP_INFO 벤처감면 적격) | ERROR |
...
|--------------|----------|-----------|----------|
| 점검 0 | 사전확인·경정청구전략 | M3-01, M3-02, M3-03, **M3-06**, **M4-10**, **M4-11** | 보완 |
| 점검 1~8 | 법인세 특화 점검 | M4-07~M4-09, **M4-12~M4-14** | 보완 |
| 점검 9 | 과세표준·세율 확인 | M1-01, STEP 1 | ○ |
| 점검 9-2 | 중소기업 판단 | M3-02 | ○ (졸업유예 상세화) |
| 점검 9-3 | 감면·공제 적용순서 | M5-05 | ○ (법인세법 §59 통일) |
| 점검 9-4 | 기존 신고 확인 | **M1-09**, RI_S_EXISTING_DEDUCTION | 신규 |
| 점검 9-5 | 환급가산금 | M6-02 | 보강 (기산일, 이율 테이블) |
| 점검 10~11 | 상시근로자 산정 | M3-04, M1-03 | ○ |
| 점검 12~13 | 창업중소기업 감면 | M4-04, M1-05 | 보강 (감면율 테이블, 인구감소지역) |
| 점검 14 | 고용공제 청년연령 | M3-04 내 청년 판단 | 연도별 분기 추가 |
| 점검 15 | 고용공제 공제액 산정 | M4-02 | 보강 (경과규정, 경력단절 등) |
| 점검 15-5~15-6 | 최저한세·이월공제 | M5-03, M5-02 | (감면초과소멸, 공제초과이월) |
| 점검 16~18 | 청년등 세부구분 | M3-04, M4-03 | (북한이탈주민, 사보청년기준) |
| 점검 19 | 고용증대세액공제 경과규정 | **M4-02 경과규정 비교** | 신규 |
| 점검 20~21 | 통합투자세액공제 | M4-01 | ○ |
| 점검 22 | R&D 세액공제 | M4-06, **RI_C_RD_EXPENSE** | 보강 (최저한세 배제 3단계) |
| 점검 23~26 | 기타 공제·감면 | M4-05, M4-07, M4-09, M4-08 | ○ |
| 점검 27 | 이월결손금 | M4-08, **RI_S_LOSS_CARRYFORWARD** | 신규 테이블 |
| 점검 28 | 사후관리 | **M7-03**, **CO_S_RISK** | 신규 |
| 점검 29 | 중간예납 | **RI_C_INTERIM_TAX**, M6-02 | 신규 |
| 점검 30 | 지방소득세 | M6-03, **M6-05** | 상세 안내 추가 |
| 점검 31 | 기업구조조정 | **M4-13** | 신규 |
| 점검 32 | 연결납세 | **M4-14** | 신규 |
...
| 사업연도별 법령 적용 | §1.3, RF_S_LAW_VERSION, RF_C_TAX_RATE_HISTORY | ✅ v2.0 신규 |
| 법인 전체 합산 | §1.3, M3-04 | ✅ |
...
| ② 소득합산 | P3-03, RI_I_OTHER_INCOME | ✅ |
| ③ 6~45% 세율 | RF_I_TAX_RATE (§5.1) | ✅ |
| ④ 소득공제 | RI_I_DEDUCTION, P4-01 | ✅ |
...
| ⑩ 지방소득세 별도 | M6-03, CO_I_LOCAL_TAX | ✅ |
| ⑪ 공동사업자 배분 | P4-03, RI_I_JOINT_BIZ | ✅ |
...
| 적용순서 (§60) | M5-05 [INC] | ✅ |
| 기존 신고 확인 (중간예납/원천징수/분납) | RI_I_BASIC | ✅ |
| 환급가산금 (비과세) | M6-02, §9.4 | ✅ |
| 다사업장 합산·배분 | P3-03, P4-02 | ✅ |
| 소득세 세율 확인 | RF_I_TAX_RATE | ✅ |
...
| 점검 33 지방소득세 | M6-03, CO_I_LOCAL_TAX | ✅ |
...
| 환급액 비교표 (소득공제 전후, 결손금 소급) | M6-04, CO_I_DEDUCTION_IMPACT | ✅ |
| 지방소득세 고정양식 | M6-03, CO_I_LOCAL_TAX | ✅ |
| 필요 증빙 서류 | M6-04 | ✅ |
| 사후관리 | M7-03 | ✅ |
| **[필수준수규칙]** |||
| ① 10원 미만 절사 | MX-05 | ✅ |
...
| ㉕ 재창업 리스크 | M4-04, BP04 | ✅ |
| ㉖ 건강보험료 가입유형 구분 | P4-01b  3분류(지역→소득공제, 직장본인→소득공제, 근로자사업주부담→필요경비) | ✅ 보완 |
| ㉗ 외국납부세액공제 | P4-07 | ✅ |
...
## 38. 법인세 입력 RI_* 24개 — 컬럼 상세 JSON 스키마 원본)

> v3.0에서 이 테이블들은 RI_S_RAW_DATA에 JSON으로 보관됩니다.
...
### 3.2 RI_C_REPRESENTATIVE — 대표자/임원/가족 정보
...
### 3.3 RI_S_EMPLOYEE_DETAIL — 상시근로자 상세 명부 *(v4.0r1 M44 보강)*
...
### 3.4 RI_S_EMPLOYEE_MONTHLY — 월별 상시근로자 요약
...
### 3.5 RI_C_INVESTMENT_ASSET — 투자 자산 명세
...
### 3.6 RI_S_STARTUP_INFO — 창업기업 세액감면 정보 *(v4.0r1 M39 벤처검증 보강)*
...
### 3.7 RI_S_SME_SPECIAL — 중소기업 특별세액감면 정보
...
### 3.8 RI_S_LOSS_CARRYFORWARD — 이월결손금 상세 *(v2.0 신규, M03)*
...
### 3.9 RI_S_CREDIT_CARRYFORWARD — 이월세액공제 상세 *(v2.0 신규, M04)*
...
### 3.10 RI_C_RD_EXPENSE — R&D 비용 상세 *(v2.0 신규, M05 / v4.0r1 E17 보강 / PK 수정)*
...
### 3.11 RI_S_EXISTING_DEDUCTION — 기존 신고 공제·감면 상세 *(v2.0 신규, M22)*
...
### 3.12 RI_C_BRANCH_LOCATION — 지점 소재지 *(v2.0 신규, M14)*
...
### 3.13 RI_C_INTERIM_TAX — 중간예납 상세 *(v2.0 신규, M15)*
...
### 3.14 RI_C_DIVIDEND_INCOME — 수입배당금 명세 *(v1.1 → v3.0 통합)*
...
### 3.15 RI_S_FOREIGN_TAX — 외국납부세액 상세 *(v4.0 신규, E13)*
...
### 3.16 RI_C_BUSINESS_VEHICLE — 업무용 승용차 *(v4.0 신규, E14)*
...
### 3.17 RI_C_TAX_ADJUSTMENT — 세무조정계산서 *(v4.0 신규, E15)*
...
### 3.18 RI_C_ENTERTAINMENT — 접대비 상세 *(v4.0 신규, M25)*
...
### 3.19 RI_C_GOVERNMENT_SUBSIDY — 국고보조금 *(v4.0 신규, E12)*
...
### 3.20 RI_C_SHAREHOLDER_LOAN — 가지급금 *(v4.0 신규, M27)*
...
### 3.21 RI_C_NON_BUSINESS_ASSET — 업무무관자산 *(v4.0 신규, M28)*
...
### 3.22 RI_C_DISASTER_LOSS — 재해손실 정보 *(v4.0r1 신규, M37)*
...
### 3.23 RI_C_CONSOLIDATED_SUB — 연결납세 자회사 *(v4.0r1 신규, M38)*
...
### 3.24 RI_C_DEPRECIATION_ADJUST — 감가상각 시부인 이월 *(v4.0r1 신규, M42)*
...
## 39. 개인사업자 전용 입력 RI_I_* 8개 — 컬럼 상세 JSON 스키마 원본)

## PART III-17. 개인사업자 전용 입력 테이블 (8개)



### 4.1 RI_I_BASIC — 개인사업자 기본 정보
...
### 4.3 RI_I_OTHER_INCOME — 사업소득 외 합산소득
...
### 4.6 RI_I_FOREIGN_TAX — 개인 외국납부세액
...
### 4.7 RI_I_RENTAL_REDUCTION — 착한 임대인
...
> v3.0에서 CO_I_* 3개는 CO_S_CREDIT_DETAIL, CO_S_REFUND에 흡수됨.
> 원본 정의를 참고용으로 수록.
...
> 법인세 출력 테이블(CO_*)은 전량 승계.

### 6.1 CO_I_INCOME_ALLOCATION — 다사업장 소득 배분 결과
...
### 6.2 CO_I_DEDUCTION_IMPACT — 소득공제 영향 분석
...
**[문서 끝]** — 통합 경정청구 환급액 산출 시스템 개발상세설계서 v4.0

> **본 문서는 자기완결적(Self-contained) 최종본**으로, 별도 참조 문서 없이 이 문서만으로 전체 시스템 설계를 파악할 수 있습니다.
